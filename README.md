...
- [main.ts](./client/main.ts) is the entrance script; it initializes the Babylon JS engine, the start menu, the Colyseus module, and the WebRTC audio chat module
- [types.ts](./client/types.ts) has the type definitions of client-only types, currently the player animcation enumerate
- [tsconfig.json](./client/tsconfig.json) has the TypeScript configuration so that the TS engine knows how to translate TS into JS
...
- [utils/](./client/utils/) has utility code; we separate it from main code to make the main code focus on game logic
- [game/createScene.ts](./client/game/createScene.ts) has the client-side game logic; depending on the state, either the start menu or the game canvas is rendered
- [audio/audio.ts](./client/audio.ts) has the WebRTC audio chat code; it exports the `audio_init()` function as module initializer
